...
- [ ] `R1-SEC-001` 更新 threat model，覆盖浏览器、控制面、数据库、对象存储、Worker、用户镜像、Node Agent、Kubernetes、dyun runner 和媒体输入信任边界。
- [ ] `R1-SEC-002` 为每种服务身份签发短期证书，支持轮换、吊销和过期诊断；内部 gRPC 强制 mTLS 和服务级授权。
- [ ] `R1-SEC-003` SecretRef 只在目标执行节点按授权解析，secret 不进入 spec、bundle、事件、日志、metrics、命令行、诊断包或数据库普通 JSON 字段。
- [ ] `R1-SEC-004` 上传和媒体输入限制大小、类型、解码资源、URL scheme、DNS/IP、重定向和超时，防止 SSRF、压缩炸弹与解析器 DoS。
- [ ] `R1-SEC-005` Worker/runner 容器无特权、无 host network/PID、最小 mount、只读 rootfs；Kubernetes admission 拒绝越权 spec。
...
- [ ] `R1-OBS-004` readiness 区分必需依赖和可选能力；健康接口不返回 secret、DSN、内部堆栈或跨租户统计。
- [ ] `R1-OBS-005` 提供 dashboard 与 alert rules：服务不可用、outbox 堆积、租约失联、训练失败率、对象错误、GPU 异常、deployment 不收敛和媒体断流。

## 3. 恢复与对账

- [ ] `R1-REC-001` 每个后台单元限定 batch、并发、deadline、retry budget 和 shutdown drain；记录 cursor/lease 后再执行下一外部副作用。
- [ ] `R1-REC-002` Training、Artifact 和 Deployment reconciler 从 desired/observed state 恢复，不依赖进程内 future 或本地队列。
- [ ] `R1-REC-003` PostgreSQL 与 MinIO 备份使用一致性窗口和 manifest；恢复后逐项验证表数量、对象摘要、引用完整性和 RLS。
- [ ] `R1-REC-004` 注入控制面、scheduler、Worker、Node Agent、dyun-agent、数据库和 MinIO 中断，记录 RTO、数据丢失和孤儿资源。
- [ ] `R1-REC-005` GC 使用引用扫描、grace period 和 dry-run；任何不确定 ownership 的对象只告警不删除。
