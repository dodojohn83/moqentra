...
- [ ] `SCOPE-001` 建立 capability → release → task 的追踪表。
- [ ] `SCOPE-002` 为每阶段冻结进入条件、退出条件、迁移和兼容承诺。
- [ ] `SCOPE-003` 定义租户管理员、数据工程师、标注员、审核员、算法工程师、运维和生态开发者旅程。
- [ ] `SCOPE-004` 定义项目、资源配额、审批和审计边界。
- [ ] `SCOPE-005` 为非目标建立 admission 流程，禁止通过局部页面绕过架构评审。
